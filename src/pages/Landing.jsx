...
const Landing = ({ isDark, setIsDark }) => {
  ...
  return (
    <div className="min-h-screen transition-colors duration-500 selection:bg-slate-900 selection:text-white dark:selection:bg-white dark:selection:text-slate-950 overflow-x-hidden relative">
      <BackgroundEffect />
      ...
      <motion.nav initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 px-6 py-3 rounded-2xl shadow-sm">
          <div className="flex items-center gap-3">
            <motion.div whileHover={{ rotate: 10, scale: 1.1 }} className="w-10 h-10 bg-slate-900 dark:bg-white rounded-xl flex items-center justify-center text-white dark:text-slate-900 shadow-lg"><Icons.Building2 size={22} /></motion.div>
          ...
          <div className="hidden md:flex items-center gap-8">{['about', 'process', 'how-to', 'contact'].map((item) => (<motion.a key={item} href={`#${item}`} whileHover={{ y: -2 }} className="text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">{item.replace('-', ' ')} System</motion.a>))}</div>
          <div className="flex items-center gap-4"><ThemeToggle isDark={isDark} setIsDark={setIsDark} /><motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Link to="/login" className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-2.5 rounded-xl font-bold text-sm shadow-xl shadow-slate-900/10 dark:shadow-none block">Access Portal</Link></motion.div></div>
        </div>
      </motion.nav>
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          <motion.div initial="hidden" animate="show" variants={staggerContainer} className="relative z-10">
            <div className="backdrop-blur-[2px] py-10 px-4 rounded-[3rem]">
              <motion.div variants={slideUp} className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100/50 dark:bg-slate-800/50 backdrop-blur-md rounded-full mb-10 border border-slate-200/50 dark:border-slate-700/50"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-teal-500"></span></span><span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 font-sans">Hostel Room And Complaint Management System</span></motion.div>
              <motion.h1 variants={slideUp} className="text-5xl md:text-7xl font-serif font-black text-slate-900 dark:text-white tracking-tight leading-[1.1]">Your Academic Journey, <br /><motion.span initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8, duration: 1 }} className="text-slate-400 dark:text-slate-500 italic">Supported by a Better Home.</motion.span></motion.h1>
              <motion.p variants={slideUp} className="max-w-2xl mx-auto text-lg md:text-xl text-slate-500 dark:text-slate-400 font-medium leading-relaxed mt-10">Experience a seamless digital ecosystem for hostel room allocation and rapid grievance resolution at CE Trikaripur.</motion.p>
              <motion.div variants={slideUp} className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-12">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Link to="/register" className="w-full sm:w-auto px-10 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 shadow-2xl shadow-slate-900/20">Apply for Admission <Icons.ArrowRight size={18} /></Link></motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Link to="/login" className="w-full sm:w-auto px-10 py-5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm">Resident Sign In</Link></motion.div>
              </motion.div>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeIn} className="py-24 px-6 bg-white dark:bg-slate-950">
      ...
      <motion.section id="how-to" initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeIn} className="py-24 px-6 bg-slate-50 dark:bg-slate-900/30">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6"><motion.div variants={slideUp} className="space-y-4"><h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">How to use the platform</h2><p className="text-slate-400 uppercase font-black tracking-[0.3em] text-[10px]">A step-by-step operational guide</p></motion.div><motion.div variants={slideUp} className="flex bg-white dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800"><button onClick={() => setActiveManual('student')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeManual === 'student' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>For Residents</button><button onClick={() => setActiveManual('warden')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeManual === 'warden' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>For Wardens</button></motion.div></div>
      ...
      <motion.section id="contact" initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeIn} className="py-32 px-6 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto">
          <motion.div variants={slideUp} className="bg-slate-50 dark:bg-slate-900/50 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 p-8 md:p-20 relative overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10">
              <div className="space-y-8"><div className="space-y-4"><h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Get in touch with us</h2><p className="text-slate-50 dark:text-slate-400 font-medium">Our administrative team is here to assist you with any housing queries or technical difficulties.</p></div><motion.div variants={staggerContainer} className="space-y-6"><ContactItem icon={Icons.Mail} label="Administrative Support" value="housing.support@cet.ac.in" /><ContactItem icon={Icons.Phone} label="Warden's Office" value="+91 467 2250311" /><ContactItem icon={Icons.MapPin} label="Office Location" value="Admin Block, CE Trikaripur, Kerala" /></motion.div></div>
              <motion.div whileHover={{ y: -5 }} className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl"><Icons.HelpCircle className="text-slate-900 dark:text-white mb-6" size={32} /><h3 className="text-xl font-bold text-slate-900 dark:text-white mb-4 font-serif">Frequently Asked?</h3><div className="space-y-4"><FAQMini question="Lost your password?" answer="Contact the Chief Warden's office with your University ID for a secure reset." /><FAQMini question="Emergency after hours?" answer="Call the 24/7 security desk at Ext. 101 for immediate assistance." /></div></motion.div>
            </div>
          </motion.div>
        </div>
      </motion.section>

      <footer className="py-12 px-6 border-t border-slate-100 dark:border-slate-900 text-center"><div className="max-w-7xl mx-auto"><motion.div animate={{ opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 4, repeat: Infinity }} className="flex items-center justify-center gap-3 mb-6 grayscale"><Icons.Building2 size={20} /><span className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Campus Housing</span></motion.div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Official System of College of Engineering Trikaripur • Designed by Group 15 • 2026</p></div></footer>
    </div>
  );
};

const FeatureMiniCard = ({ title, desc }) => (<motion.div variants={{ hidden: { y: 20, opacity: 0 }, show: { y: 0, opacity: 1 } }} whileHover={{ y: -5, borderColor: 'rgb(20, 184, 166)' }} className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 transition-all"><Icons.CheckCircle2 className="text-teal-500 mb-3" size={24} /><h4 className="font-bold text-slate-900 dark:text-white mb-1">{title}</h4><p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{desc}</p></motion.div>);
const ProcessStep = ({ number, icon: Icon, title, desc }) => (<motion.div variants={{ hidden: { y: 30, opacity: 0 }, show: { y: 0, opacity: 1 } }} className="relative p-10 bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200/60 dark:border-slate-800 shadow-sm group hover:shadow-xl transition-all duration-500 hover:-translate-y-2"><div className="absolute top-6 right-8 text-4xl font-black text-slate-100 dark:text-slate-800 transition-colors group-hover:text-slate-200 dark:group-hover:text-slate-700">{number}</div><div className="w-14 h-14 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl flex items-center justify-center mb-8 shadow-lg group-hover:rotate-12 transition-transform"><Icon size={24} /></div><h3 className="text-xl font-black text-slate-900 dark:text-white mb-4 leading-tight">{title}</h3><p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">{desc}</p></motion.div>);
...
const ManualCard = ({ icon: Icon, title, desc }) => (<motion.div whileHover={{ y: -5, scale: 1.02 }} className="p-10 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group"><div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform"><Icon size={24} /></div><h4 className="font-black text-slate-900 dark:text-white uppercase tracking-wider text-sm mb-3">{title}</h4><p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">{desc}</p></motion.div>);
const ContactItem = ({ icon: Icon, label, value }) => (<motion.div variants={{ hidden: { y: 10, opacity: 0 }, show: { y: 0, opacity: 1 } }} className="flex items-center gap-5"><div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-900 dark:text-white shadow-sm"><Icon size={20} /></div><div><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-0.5">{label}</p><p className="text-sm font-bold text-slate-900 dark:text-white">{value}</p></div></motion.div>);
const FAQMini = ({ question, answer }) => (<div className="space-y-2"><div className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-teal-500" />{question}</div><p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed pl-3.5">{answer}</p></div>);
