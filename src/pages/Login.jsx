...
const Login = ({setUser}) => {
 ...
 <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-xl transform hover:rotate-12 transition-transform"><Icons.Building2 size={24} /></div>
 <div><h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Portal <span className="text-slate-400 font-medium">Access</span></h1><p className="text-slate-500 font-medium text-[13px]">Authorized residential entry</p></div>
 </div>

 <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-100">
 {['student','admin'].map(r => (
 <button key={r} onClick={() => setRole(r)} className={`flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all duration-300 flex items-center justify-center gap-2 ${role === r ?'bg-white shadow-lg text-slate-900':'text-slate-400 hover:text-slate-600'}`}>
 ...
 <div className="pt-6 border-t border-slate-50 text-center"><p className="text-[11px] font-bold text-slate-400">New to College Hostel? <Link to="/register"className="text-slate-900 font-black hover:underline decoration-2 underline-offset-4 ml-1">Register Now</Link></p></div>
 </div>
 </div>
 <div className="p-6 text-center"><p className="text-[8px] font-black uppercase tracking-[0.3em] text-slate-200">Campus Housing • Secure Protocol • Group 15</p></div>
 </div>
 </div>
 );
};
