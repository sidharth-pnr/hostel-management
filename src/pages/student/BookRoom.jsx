    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      {studentStatus?.room_rejection_note && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-5xl mx-auto bg-red-500/10 border border-red-500/20 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-10">
             ...
             <div className="space-y-3 flex-1"><p className="text-[10px] font-black uppercase tracking-widest text-red-500">Notice from Warden</p><h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Request Rejected.</h3><div className="mt-4 bg-white/40 dark:bg-slate-950/40 p-6 rounded-3xl border border-red-500/10 backdrop-blur-md"><p className="text-sm font-bold text-slate-600 dark:text-slate-300 italic leading-relaxed">"Reason: {studentStatus.room_rejection_note}"</p></div></div>
             <button onClick={() => axios.post(`${API_BASE}/admin_action.php`, { action: 'dismiss_rejection', student_id: studentId }).then(() => fetchData())} className="px-10 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-2xl hover:scale-105 transition-all">Dismiss</button>
          </div>
        </motion.div>
      )}
      ...
      <div className="max-w-7xl mx-auto space-y-12 px-4 sm:px-6">
        ...
        <section className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 relative w-full group"><Icons.Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={20} /><input type="text" placeholder="Search room number or block..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-2xl py-5 pl-16 pr-6 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all placeholder:text-slate-400 shadow-sm" /></div>
                  ...
                  <motion.div key={r.room_id} layout className="group">
                    <GlassCard p="p-0" className="flex flex-col h-full hover:border-blue-500/30 transition-all duration-500">
                      <div className="p-8 md:p-10 flex-1 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-10 relative z-10"><div className="p-5 bg-blue-600/10 dark:bg-white/5 text-blue-600 dark:text-white rounded-2xl group-hover:scale-110 transition-transform duration-500 shadow-inner"><Icons.BedDouble size={32} /></div><div className="flex flex-col items-end gap-2"><span className="bg-blue-500/10 text-blue-600 dark:text-blue-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-500/20 shadow-sm">CASH {r.price}</span><span className="bg-teal-500/10 text-teal-600 dark:text-teal-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-teal-500/20 shadow-sm">{r.capacity - r.current_occupancy} Free Slots</span></div></div>
                        <div className="relative z-10 space-y-2 mb-10"><p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Room Number</p><h3 className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">ROOM {r.room_number}</h3><div className="flex items-center gap-2 pt-2 text-slate-500 dark:text-slate-400 font-bold text-[11px] uppercase tracking-widest"><Icons.Building2 size={14} className="text-blue-500" /><span>Block {r.block} • Standard Room</span></div></div>
                        <div className="space-y-4 pt-8 border-t border-slate-100 dark:border-white/5 relative z-10">
                          <div className="flex justify-between items-end"><div className="space-y-1"><span className="text-slate-400 font-black uppercase text-[9px] tracking-widest block">Room Occupancy</span><p className="font-black text-slate-900 dark:text-white text-base tracking-tight">{r.current_occupancy} / {r.capacity} OCCUPIED</p></div><span className="text-2xl font-black text-blue-500 tracking-tighter leading-none">{((r.current_occupancy / r.capacity) * 100).toFixed(0)}%</span></div>
                          <div className="w-full bg-slate-100 dark:bg-white/5 h-2.5 rounded-full overflow-hidden shadow-inner"><motion.div initial={{ width: 0 }} animate={{ width: `${(r.current_occupancy / r.capacity) * 100}%` }} transition={{ duration: 1 }} className={`h-full rounded-full ${r.current_occupancy >= r.capacity ? 'bg-slate-900 dark:bg-white' : 'bg-gradient-to-r from-blue-600 to-indigo-500'}`} /></div>
                        </div>
                      </div>
                      <button onClick={() => setSelectedRoomForApply(r)} disabled={user?.account_status === 'PENDING'} className={`w-full py-8 font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-4 transition-all relative overflow-hidden group/btn ${user?.account_status === 'PENDING' ? 'bg-slate-100 dark:bg-slate-800/50 text-slate-400 cursor-not-allowed opacity-50' : 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100'}`}><div className="absolute inset-0 bg-blue-500/10 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500" /><span className="relative z-10">{user?.account_status === 'PENDING' ? "Waiting for Approval" : (studentStatus?.room_id ? "Request Change" : "Book Room")}</span>{user?.account_status !== 'PENDING' && <Icons.ArrowRight size={18} className="relative z-10 group-hover:translate-x-2 transition-transform" />}</button>
                    </GlassCard>
                  </motion.div>
        ...
        {selectedRoomForApply && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 md:p-12 bg-slate-950/60 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}>
              <GlassCard className="w-full max-w-2xl relative overflow-hidden" p="p-10 md:p-12">
                <div className="space-y-10">
                  <div className="flex justify-between items-start"><div className="space-y-2"><p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500">Room Request</p><h3 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">{studentStatus?.room_id ? "Change Room." : "Book Room."}</h3></div><div className="p-4 bg-blue-600/10 rounded-2xl text-blue-600 font-black text-2xl tracking-tighter flex flex-col items-end"><span>R-{selectedRoomForApply.room_number}</span><span className="text-[10px] font-black uppercase tracking-widest text-blue-400 mt-1">CASH {selectedRoomForApply.price}</span></div></div>
                  <div className="space-y-6">
                     {studentStatus?.room_id ? (
                       <div className="space-y-3"><label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1"><Icons.FileText size={14} />Reason for Request</label><textarea value={requestReason} onChange={(e) => setRequestReason(e.target.value)} placeholder="Please explain why you want to change to this room..." className="w-full min-h-[180px] p-6 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 rounded-3xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all resize-none placeholder:text-slate-400 shadow-sm" /></div>
                     ) : <div className="p-8 bg-blue-500/5 rounded-[2rem] border border-dashed border-blue-500/20 text-center space-y-2"><p className="text-sm font-bold text-slate-600 dark:text-slate-300">Ready to book this room?</p><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Click below to submit your request to the warden.</p></div>}
                     <div className="flex items-center gap-4 bg-blue-500/5 p-5 rounded-2xl border border-blue-500/10"><Icons.Info size={20} className="text-blue-500 shrink-0" /><p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 leading-relaxed uppercase tracking-wider">Note: All requests must be approved by the warden.</p></div>
                  </div>
                  <div className="flex gap-4 pt-4"><button onClick={() => setSelectedRoomForApply(null)} className="flex-1 py-5 rounded-2xl font-black uppercase tracking-widest text-[11px] text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">Cancel</button><button onClick={requestRoom} className="flex-[2] bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-5 rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"><Icons.ArrowUpRight size={18} />Submit Request</button></div>
                </div>
              </GlassCard>
            </motion.div>
          </div>
