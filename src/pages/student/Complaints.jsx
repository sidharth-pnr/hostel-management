...
const Complaints = () => {
  ...
  const closeComplaint = async (id) => {
    try {
      const res = await axios.put(`${API_BASE}/complaints.php`, { 
        complaint_id: id, 
        status: 'CLOSED',
      ...
      if (res.data.status === 'success') {
        ...
        toast.error(res.data.error || "Failed to close");
      }
    } catch (err) {
      toast.error("Connection error");
    }
  ...
  return (
    <div className="space-y-12 pb-20 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative">
        <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-slate-200 dark:border-white/10 shadow-2xl p-8 sm:p-12 overflow-hidden">
          <div className="relative z-10 space-y-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 dark:border-white/5 pb-10">
               ...
               <p className="text-xs font-bold text-slate-400 dark:text-slate-500 max-w-sm uppercase tracking-widest leading-relaxed">
                 Tell us about any problem in your room or hostel.
               </p>
            </div>
            ...
            <form onSubmit={handleComplaint} className="space-y-10">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                  {categories.map((cat) => (
                    <button
                      key={cat.id} type="button" onClick={() => setSelectedCategory(cat.id)}
                      className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center gap-3 relative overflow-hidden ${
                        selectedCategory === cat.id
                          ? `bg-${cat.color}-500/10 border-${cat.color}-500 shadow-lg scale-105`
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <input name="title" placeholder="Briefly describe the issue" required className="w-full p-5 bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 text-slate-900 dark:text-white rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-sm shadow-sm" />
                </div>
                ...
                <div className="md:col-span-2 flex justify-end gap-6 pt-4">
                  <button type="button" onClick={() => navigate('/student')} className="px-8 py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.3em] text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">Cancel</button>
                  <button 
      ...
      <section className="space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 dark:border-white/10 pb-8 px-4">
           <div className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400">Past Complaints</span>
              <h3 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Complaints History.</h3>
           </div>
           <div className="flex items-center gap-3 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl p-1.5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-x-auto no-scrollbar max-w-full">
              ...
              <FilterTab active={filter === 'RESOLVED'} onClick={() => setFilter('RESOLVED')} label="Fixed" />
              <FilterTab active={filter === 'CLOSED'} onClick={() => setFilter('CLOSED')} label="Closed" />
           </div>
        </div>
...
const StatNode = ({ label, value, icon: Icon, color }) => (
  <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-lg relative group">
    <div className="relative z-10 space-y-4">
       <div className="w-12 h-12 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center shadow-lg"><Icon size={24} /></div>
       <div>
...
const PriorityIndicator = ({ level }) => {
  ...
  return (
    <div className={`flex flex-col items-center gap-1 p-3 rounded-2xl border border-transparent ${color}`}>
       <Icon size={18} />
