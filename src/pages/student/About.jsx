...
const StudentAbout = () => {
 ...
 <motion.section variants={slideUp} initial="hidden"animate="show"className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden group">
 ...
 <div className="flex items-center gap-4 pt-4"><div className="h-1 flex-1 bg-white/20 rounded-full overflow-hidden"><motion.div initial={{width: 0}} animate={{width:'100%'}} transition={{duration: 2}} className="h-full bg-blue-500"/></div><span className="text-[10px] font-black tracking-widest">SECURED</span></div>
 </div>
 </motion.section>
...
const PortalLink = ({label}) => (
 <a href="#"className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-100 transition-all group">
 <span className="text-xs font-black text-slate-600 group-hover:text-slate-900 uppercase tracking-widest transition-colors flex items-center gap-3"><Icons.ChevronRight size={14} className="text-blue-500 group-hover:translate-x-1 transition-transform"/>{label}</span>
 <Icons.ArrowUpRight size={16} className="text-slate-300 group-hover:text-blue-500 transition-colors"/>
 </a>
);
