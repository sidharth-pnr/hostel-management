...
const StudentOverview = () => {
  ...
  return (
    <>
      ...
      {/* --- PAYMENT MODAL - Moved outside the animated wrapper to prevent clipping --- */}
      <AnimatePresence>
        {showPaymentModal && (
              ...
              <div className="p-6 sm:p-8 overflow-y-auto scrollbar-hide space-y-8">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <button 
                    ...
                    className="w-full py-2 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                  >
                    Cancel Transaction
                  </button>
                </div>
...
const PaymentOption = ({ active, onClick, icon: Icon, label, desc }) => (
  ...
  >
    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${active ? 'bg-blue-500 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>
      <Icon size={16} />
    </div>
