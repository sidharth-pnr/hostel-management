...
const Profile = () => {
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-500 w-full">
      ...
      {/* 1. IDENTITY HEADER */}
      <section className="flex flex-col md:flex-row items-center gap-8 py-6 border-b border-slate-100 dark:border-white/5">
        <div className="relative">
          ...
          <div className="absolute -bottom-1 -right-1 bg-emerald-500 p-1.5 rounded-lg border-2 border-white dark:border-slate-950 shadow-lg"><Icons.ShieldCheck size={14} className="text-white" /></div>
        </div>

        <div className="flex-1 text-center md:text-left space-y-3">
          <div className="flex flex-wrap justify-center md:justify-start gap-2">
            <span className="px-3 py-1 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-slate-200 dark:border-white/5">My Personal Info</span>
          ...
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">{user.name}</h1>
          <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-400">Student Resident • Hostel Management • Group 15</p>
        </div>

        {!isEditing && <button onClick={() => setIsEditing(true)} className="px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-black uppercase tracking-widest text-[9px] shadow-lg hover:scale-105 transition-all flex items-center gap-2"><Icons.Pencil size={14} /> Edit Profile</button>}
      </section>
      ...
      <AnimatePresence mode="wait">
        {!isEditing ? (
          <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <InfoNode icon={Icons.GraduationCap} label="Department" value={user.department} color="blue" className="p-5 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10" />
            <InfoNode icon={Icons.Clock} label="Year of Study" value={`Year ${user.year}`} color="teal" className="p-5 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10" />
            <InfoNode icon={Icons.Phone} label="Phone Number" value={user.phone || 'Not Set'} color="indigo" className="p-5 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10" />
            <InfoNode icon={Icons.Hash} label="Student ID" value={user.reg_no} color="slate" className="p-5 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10" />
          </motion.div>
        ) : (
          <motion.form key="edit" initial={{ opacity: 0, scale: 0.99 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} onSubmit={handleUpdate} className="space-y-6">
            <GlassCard>
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-4 mb-6">
              ...
              <div className="flex justify-end gap-4 pt-6 mt-6 border-t border-slate-100 dark:border-white/5"><button type="button" onClick={() => setIsEditing(false)} className="px-6 py-3 rounded-lg font-black text-[9px] uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white">Cancel</button><button type="submit" className="px-8 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-black uppercase tracking-widest text-[9px] shadow-lg flex items-center gap-2"><Icons.CheckCircle2 size={14} className="text-blue-500" /> Save Changes</button></div>
            </GlassCard>
          </motion.form>
        )}
      </AnimatePresence>
      ...
      <div className="min-h-[100px]">
        {room && room.room_number ? (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={() => navigate('/student/book')} className="cursor-pointer group bg-slate-50 dark:bg-white/5 p-6 rounded-[2rem] border border-slate-200 dark:border-white/10 flex items-center justify-between gap-6 hover:border-blue-500/30 transition-all">
            <div className="flex items-center gap-6"><div className="w-16 h-16 rounded-2xl bg-blue-600/10 flex items-center justify-center text-blue-600 border border-blue-600/20 group-hover:scale-110 transition-transform"><Icons.BedDouble size={32} /></div><div><p className="text-[9px] font-black uppercase tracking-widest text-blue-500">My Room Info</p><h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Room {room.room_number}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Block {room.block} • Standard Room</p></div></div>
        ...
        ) : <div className="h-[100px] flex items-center justify-center border border-dashed border-slate-200 dark:border-white/5 rounded-[2rem] opacity-40"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Loading your room info...</p></div>}
      </div>
    </div>
  );
};
