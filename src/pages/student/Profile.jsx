    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-500 w-full">
      ...
      <section className="flex flex-col md:flex-row items-center gap-8 py-6 border-b border-slate-100 dark:border-white/5">
          ...
          <div className="w-32 h-32 rounded-3xl bg-slate-100 dark:bg-white/5 p-1 border border-slate-200 dark:border-white/10 shadow-xl overflow-hidden group">
            <div className="w-full h-full rounded-[1.25rem] bg-white dark:bg-slate-900 flex items-center justify-center relative overflow-hidden">
              ...
              <motion.div 
                ...
                className="absolute left-0 right-0 h-0.5 bg-blue-500/20 blur-sm z-20"
              />
            </div>
          </div>
        ...
        <div className="flex-1 text-center md:text-left space-y-3">
          <div className="flex flex-wrap justify-center md:justify-start gap-2">
            ...
            <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${user.account_status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
              Account {user.account_status === 'ACTIVE' ? 'Active' : 'Pending'}
            </span>
          </div>
      ...
      <AnimatePresence mode="wait">
        {!isEditing ? (
          ...
          <motion.form 
            key="edit" initial={{ opacity: 0, scale: 0.99 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            onSubmit={handleUpdate}
            className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-xl space-y-6"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full p-4 bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-xs"
                >
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Year of Study</label>
                <select 
                  value={editData.year} 
      ...
      <div className="min-h-[100px]">
        {room && room.room_number ? (
          <motion.div 
            ...
            className="cursor-pointer group bg-slate-50 dark:bg-white/5 p-6 rounded-[2rem] border border-slate-200 dark:border-white/10 flex items-center justify-between gap-6 hover:border-blue-500/30 transition-all"
          >
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Room {room.room_number}</h3>
          ...
          <div className="h-[100px] flex items-center justify-center border border-dashed border-slate-200 dark:border-white/5 rounded-[2rem] opacity-40">
             <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Loading your room info...</p>
          </div>
        )}
      </div>
...
const MiniNode = ({ icon: Icon, label, val, color }) => (
  <div className="p-5 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
    <div className={`w-10 h-10 rounded-xl bg-${color}-500/10 flex items-center justify-center text-${color}-500 shadow-inner group-hover:scale-110 transition-transform`}>
      <Icon size={20} />
    </div>
    <div>
...
const InputGroup = ({ label, value, onChange, icon: Icon }) => (
  <div className="space-y-2">
    ...
    <div className="relative group">
      <Icon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={16} />
      <input 
        type="text" value={value} onChange={onChange} required
        className="w-full p-4 pl-12 bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-xs"
      />
    </div>
  </div>
);
