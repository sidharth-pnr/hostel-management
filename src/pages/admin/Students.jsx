      ...
      <div className="space-y-8 animate-slide-up relative">
        ...
        <div className="flex flex-col xl:flex-row gap-6 items-stretch xl:items-center">
          <div className="relative flex-1 group">
            <Icons.Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-focus-within:text-slate-900 dark:group-focus-within:text-white transition-colors" size={20}/><input placeholder="Search by name or registration ID..." className="w-full pl-12 pr-6 py-4 bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800 outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-white transition-all text-slate-900 dark:text-white" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        ...
        <div className="space-y-4">
          <div className="hidden lg:grid grid-cols-12 px-8 text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em] mb-2"><div className="col-span-5">Scholar Identity</div><div className="col-span-3 text-center">Accommodation Status</div><div className="col-span-4 text-right">Operational Actions</div></div>
...
const ResidentCard = ({ student, availableRooms, handleRoomAction, onRejectRequest, userRole }) => {
  ...
  const getStatusUI = () => {
    if (['REQUESTED', 'APPROVED'].includes(room_request_status)) {
      return (
        <div className="flex flex-col items-center">
          <span className={`flex items-center gap-1.5 px-3 py-1 ${room_request_status === 'APPROVED' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400'} rounded-full text-[9px] font-black uppercase tracking-wider mb-2`}>
    ...
    if (room_request_status === 'SUGGESTED') return <div className="flex flex-col items-center"><span className="flex items-center gap-1.5 px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-[9px] font-black uppercase tracking-wider mb-2"><Icons.ArrowRight size={10} /> Offer Pending</span></div>;
    if (current_room_no) return <div className="flex flex-col items-center"><span className="flex items-center gap-1.5 px-3 py-1 bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400 rounded-full text-[9px] font-black uppercase tracking-wider mb-2"><Icons.MapPin size={10} /> Room {current_room_no}</span></div>;
    return <div className="flex flex-col items-center"><span className="px-3 py-1 bg-slate-100 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 rounded-full text-[9px] font-black uppercase tracking-wider mb-2">No Assignment</span></div>;
  };

  return (
    <div className="flex flex-col gap-4">
      <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm p-6 lg:p-8 rounded-[2.5rem] border border-slate-200/60 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group grid grid-cols-1 lg:grid-cols-12 items-center gap-6">
        <div className="lg:col-span-5 flex items-center gap-6">
          <div className="w-16 h-16 rounded-[1.5rem] bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center font-black text-2xl text-slate-900 dark:text-white transition-transform group-hover:scale-110">{(student.name || "?").charAt(0)}</div>
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2"><span className="px-2 py-0.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-md text-[9px] font-black tracking-widest uppercase">{student.reg_no || "No ID"}</span><span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">{student.department} • Year {student.year}</span></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="flex flex-col gap-3 items-end">
            ...
            {showSuggest && (
              <div className="flex gap-2 p-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl">
                <select className="text-[10px] font-bold p-2 bg-slate-50 dark:bg-slate-800 border-none text-slate-900 dark:text-white rounded-xl outline-none" onChange={(e) => setSelectedRoom(e.target.value)} value={selectedRoom}>
                  <option value="">Choose Room...</option>
                  {availableRooms.map(r => <option key={r.room_id} value={r.room_id}>{r.room_number} ({r.current_occupancy}/{r.capacity})</option>)}
                </select>
                <button disabled={!selectedRoom} onClick={() => { handleRoomAction('suggest_room', student_id, null, selectedRoom); setShowSuggest(false); }} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-1 rounded-xl text-[10px] font-black uppercase disabled:opacity-30">Send</button>
              </div>
            )}
          </div>
        </div>
      ...
      {['REQUESTED', 'APPROVED'].includes(room_request_status) && room_request_reason && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mx-8 -mt-2 p-6 bg-blue-500/5 dark:bg-blue-500/10 rounded-b-[2rem] border-x border-b border-blue-500/20 flex items-start gap-4">
          <div className="p-3 bg-blue-500/10 rounded-xl text-blue-500"><Icons.FileText size={18} /></div>
          <div><p className="text-[9px] font-black uppercase tracking-widest text-blue-500/60 mb-1">Scholar's Statement of Purpose</p><p className="text-sm font-bold text-slate-600 dark:text-slate-300 italic">"{room_request_reason}"</p></div>
        </motion.div>
      )}
    </div>
  );
