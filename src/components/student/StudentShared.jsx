...
export const GlassCard = ({children, className ="", p ="p-8 sm:p-10"}) => (
 <div className={`bg-white/40 backdrop-blur-[40px] rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden ${className}`}>
 <div className={p}>
 {children}
 </div>
 </div>
...
export const PageHeader = ({title, subtitle, badge, badgeIcon: BadgeIcon, badgeColor ="blue"}) => (
 <section className="relative py-8 border-b border-slate-100">
 <div className="space-y-6">
 {badge && (
 <div className="flex flex-wrap items-center gap-3">
 <span className={`px-3 py-1 bg-${badgeColor}-500/10 text-${badgeColor}-600}-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-${badgeColor}-500/20 flex items-center gap-2`}>
...
export const StatNode = ({label, value, icon: Icon, color ="blue"}) => {
 ...
 return (
 <div className="bg-white/40 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-slate-200 shadow-lg relative group">
...
export const InfoNode = ({icon: Icon, label, value, color ="blue", className =""}) => (
 ...
 <div className={`w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 group-hover:text-${color}-500 transition-colors shadow-inner`}>
 <Icon size={18} />
 </div>
 <div>
 <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-0.5">{label}</p>
 <p className="text-sm font-bold text-slate-700 uppercase tracking-tight">{value}</p>
 </div>
 </div>
);
...
export const EmptyState = ({icon: Icon = Icons.Box, title ="No Records Found.", message ="Everything is clear for now."}) => (
 <div className="col-span-full py-32 text-center bg-white/20 rounded-[4rem] border border-dashed border-slate-300 shadow-inner">
 <Icon size={64} className="mx-auto text-slate-300 mb-8 animate-pulse"/>
