...
export const StatCard = ({icon: Icon, label, value, subValue, color, pulse, progress}) => {
 ...
 return (
 <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200/60 shadow-sm relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
...
export const EmptyState = ({icon: Icon = Icons.Activity, title ="All Clear", message ="No records found matching your criteria"}) => (
 ...
 <Icon size={48} strokeWidth={1} className="mx-auto mb-4 text-slate-300"/>
 <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase mb-1">{title}</h3>
...
export const FilterMenu = ({icon: Icon, value, onChange, children}) => (
 <div className="bg-white px-5 py-4 rounded-2xl border border-slate-200 flex items-center gap-3 shadow-sm hover:border-slate-900 transition-all group">
