...
export const Input = ({ label, icon: Icon, error, className = "", ...props }) => (
  <div className={`space-y-1.5 ${className}`}>
    ...
    <div className="relative group">
      ...
      <input 
        className={`w-full p-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/50 text-slate-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-white transition-all font-bold text-sm ${Icon ? 'pl-12' : ''} ${error ? 'border-red-500' : ''}`}
        {...props}
      />
    </div>
...
export const Select = ({ label, options = [], className = "", ...props }) => (
  <div className={`space-y-1.5 ${className}`}>
    ...
    <select 
      className="w-full p-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/50 text-slate-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-white transition-all font-bold text-sm appearance-none cursor-pointer"
      {...props}
    >
      {options.map(opt => (
...
export const PrimaryButton = ({ children, isLoading, loadingText, icon: Icon = Icons.ArrowRight, className = "", ...props }) => (
  <button 
    disabled={isLoading}
    className={`w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-slate-900/10 disabled:opacity-50 ${className}`}
    {...props}
  >
