...
The application features a cutting-edge UI/UX strategy to ensure a premium academic experience:

### 1. Glassmorphism (Visual Depth)
- **Implementation:** Extensive use of `backdrop-blur`, semi-transparent backgrounds (`bg-white/40`), and thin white borders to create a layered, "frosted glass" effect.
- **Components:** Seen in the `GlassSidebar`, `GlassBox`, and mobile overlays.
- **Goal:** Provides a lightweight, modern feel that reduces visual clutter while maintaining high contrast.

### 2. Bento Box Architecture (Information Hierarchy)
- **Implementation:** A modular grid layout inspired by Apple's "Bento" style. Each functional unit (Metric Cards, Infrastructure Heatmap, Resolution Map) is contained within its own distinct `GlassBox`.
- **Dashboard:** The Admin and Student overviews use a responsive `grid-cols-4` layout to organize complex data into digestible, high-impact blocks.
- **Goal:** Simplifies data-heavy interfaces and ensures a consistent, organized user experience across different screen sizes.
...
- **Overview Dashboard:** Real-time stats on occupancy, student count, and urgent complaints using the **Bento Grid** layout.
...
- **Aesthetics & Ergonomics:** High-end UI with minimal friction and professional layout.
